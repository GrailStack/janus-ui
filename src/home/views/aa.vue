        ...
        <div class="echarts-list">
          ...
          <div class="monitor-list">
            <el-radio-group v-model="monitorCheckedRadio">
              <el-radio v-for="(item, index) in viewGatewayStatusList" v-show="item.zuulStatus !== 'Dead'" :label="item.zuulInstanceId" :key="index">{{item.zuulInstanceId}}</el-radio>
            </el-radio-group>
          </div>
        </div>
      </div>
...
export default {
  ...
  created () {
    this.roleList = JSON.parse(sessionStorage.getItem('selectAuth')).indexOf('admin')
    this.getPandectdata()
  },
  mounted () {
    this.getApiHealth()
  ...
  methods: {
    ...
    setPieCharts: function (params) {
      ...
      myChart.setOption({
        title: {
          text: '已使用内存:' + params['mem.free'],
          subtext: 'MEM:' + params['mem'], // 子标题
          x: 'center',
          bottom: -5,
          textStyle: {
            fontWeight: 'normal',
        ...
        series : [
          {
            ...
            data:[
              {value: params['mem.free'], name:'已使用内存'},
              {value: params['mem'] - params['mem.free'], name:'剩余内存'},
            ],
            itemStyle: {
              emphasis: {
                shadowBlur: 10,
    ...
    setDashboardEcharts: function (params) {
      ...
      myChart.setOption({
        title: {
          text: 'CPU负载:' + params['systemload.average'],
          subtext: 'CPU核数:' + params['processors'], // 子标题
          x: 'center',
          bottom: -5,
          textStyle: {
            fontWeight: 'normal',
        ...
        series: [
          {
            ...
            data: [{name: '使用率', value: params['systemload.average'] * 100}] // 使用---------
          }
        ]
      })
    },
    ...
    apiHealth: function (params) {
      let colorList = ['#946CFF', '#F26186', '#FFB029', '#E91E63', '#2196F3']
      let seriesList = []
    ...
    invocationTrend: function (params) {
      let colorList = ['#FFB029', '#946CFF', '#F26186', '#E91E63', '#2196F3']
      let seriesList = []
      let data = Object.keys(params)
